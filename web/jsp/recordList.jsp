...
<script type="text/javascript">
    $(document).ready(function(){
        $('.form_date').datetimepicker({
            ...
            forceParse: 0,/*就是你输入的可能不正规，但是它会强制尽量解析成你规定的格式（format）  */
...
<div class="data_list">
    ...
    <form name="myForm"  action="record.action?action=list"  class="form-search" method="post"  style="padding-bottom: 0px">
        <c:if test="${session_user.roleId != 2 }">
            <button class="btn btn-success" type="button" style="margin-right: 50px;" onclick="window.location='record.action?action=preAdd'">添加</button>
        </c:if>
        ...
        <table class="table table-striped table-bordered table-hover datatable">
            ...
            <c:forEach items="${records}"  var="record">
                <tr>
                    ...
                    <c:if test="${session_user.roleId != 2 }">
                        <td>
                            <button class="btn btn-mini btn-success" type="button" onclick="window.location='record.action?action=preUpdate&id=${record.id }'">修改</button>
                            <c:if test="${record.disabled == 0}">
                                <button class="btn btn-mini btn-danger" type="button" onclick="deleteOrActive(${record.id },1)">删除</button>
                            </c:if>
                            <c:if test="${record.disabled == 1}">
                                <button class="btn btn-mini btn-danger" type="button" onclick="deleteOrActive(${record.id },0)">激活</button>
                            </c:if>
                        </td>
                    </c:if>
                </tr>
